const isUpdateProductFormValid = (req, res, next) => {
  try {
    ...
    const mainImagesIndices = req.body.mainImagesIndices ? 
      (Array.isArray(req.body.mainImagesIndices) ? req.body.mainImagesIndices : [req.body.mainImagesIndices]) : [];
    const highlightImagesIndices = req.body.highlightImagesIndices ? 
      (Array.isArray(req.body.highlightImagesIndices) ? req.body.highlightImagesIndices : [req.body.highlightImagesIndices]) : [];
    ...
    if (!status) {
      ...
      errors.others.push(`Invalid product status. Must be one of: ${validStatuses.join(', ')}`);
    }
    ...
    if (totalMainImages !== expectedCounts.main) {
      errors.images.push(`Total of ${expectedCounts.main} product images required (currently ${totalMainImages})`);
    }
    ...
    if (totalHighlightImages !== expectedCounts.highlight) {
      errors.highlightImg.push(`Total of ${expectedCounts.highlight} highlight images required (currently ${totalHighlightImages})`);
    }
