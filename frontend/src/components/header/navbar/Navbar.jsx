...
const Navbar = ({
    ...
    return (
        <header className='sticky top-0 left-0 z-999 bg-black flex justify-between items-center text-2xl w-full py-4 border-t border-zinc-700 px-5 print:hidden' id="mobile-navbar">
            ...
            {/* We do NOT hide this anymore on mobile, the search overlay covers it cleanly */}
                    ...
                    {links.map(({ title, link }) => (
                        <li key={`${title}-desktop-nav`}>
                            <NavLink
                                to={link}
                                className={({ isActive }) =>
                                    `capitalize hover:text-white transition-colors ${isActive ? "text-red-600 font-bold" : ""}`
                                }
                            >
                                {title}
                            </NavLink>
            ...
            {/* Right Side: Icons */}
            {/* "static" on mobile allows the absolute search child to cover the parent Header. "relative" on desktop keeps it inline. */}
            <nav className="nav-icons flex gap-5 lg:w-auto justify-end items-center static lg:relative w-[30%]">
                    ...
                    {isSearchOpen ? (
                        <div className="w-full relative lg:w-[400px] animate-in fade-in zoom-in-95 duration-200">
                            ...
                            <div className="relative group flex items-center gap-2">
                                ...
                                {/* Cancel Button (Mobile Only Text, Desktop Icon) */}
                                <button 
                                    onClick={closeSearch} 
                                    className="lg:absolute lg:right-3 lg:top-1/2 lg:-translate-y-1/2 lg:p-1.5 lg:hover:bg-zinc-800 lg:rounded-full transition-all text-sm font-medium text-zinc-400 hover:text-white whitespace-nowrap px-2"
                                >
                                            ...
                                            <ul className="divide-y divide-zinc-900/50">
                                                {searchResults.map((product) => (
                                                    <li 
                                                        ...
                                                        className="cursor-pointer hover:bg-white/5 transition-all duration-200 p-3 flex gap-4 items-center group relative"
                                                    >
                                                        {/* Image */}
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex justify-between items-start gap-2">
                                                                <h4 className="text-sm font-semibold text-zinc-200 group-hover:text-red-500 transition-colors truncate">
                                                                    {product.name}
                                                                </h4>
                                                                <span className="text-xs font-bold text-white bg-zinc-800/80 px-2 py-0.5 rounded-sm shrink-0 whitespace-nowrap">
                                                                    Rs. {product.price}
                                                                </span>
                                                            </div>
                                                        ...
                                                        {/* Hover Arrow */}
                                                        <div className="text-zinc-500 opacity-0 -translate-x-2 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300 hidden sm:block">
                        ...
                        <span
                            ...
                            className="cursor-pointer text-zinc-300 hover:text-white transition-all text-2xl w-10 h-10 flex items-center justify-center rounded-full hover:bg-zinc-900 active:scale-95"
                        >
                ...
                {/* Hidden when search is OPEN on MOBILE to prevent clutter */}
                <div className={`hidden lg:block relative ${isSearchOpen ? 'hidden lg:block' : ''}`} ref={dropdownRef}>
                    ...
                    >
                        <Icon
                            icon={admin ? "solar:shield-user-outline" : "iconamoon:profile-light"}
                            className="text-2xl"
                        />
                    </div>
                                ...
                                <>
                                    ...
                                    {userDropdownLinks.map((item) => (
                                        <Link
                                            ...
                                            className="flex items-center gap-3 px-4 py-2.5 hover:bg-zinc-800 text-sm text-zinc-300 hover:text-white transition-colors"
                                        >
                                    ...
                                    ))}
                                    {
                                        !user &&
                                        <>
                                            <Link to="/account/signin" onClick={() => setIsUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-900 text-sm text-zinc-300 hover:text-white transition-colors">
                                                <Icon icon="solar:login-3-bold" className="text-lg" />
                                                Login / Sign Up
                                            </Link>
                                            <Link to="/admin/signin" onClick={() => setIsUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-900 text-sm text-zinc-300 hover:text-white transition-colors border-t border-zinc-900">
                                    ...
                                    }
                                    {
                                        ...
                                        <div className="border-t border-zinc-800 mt-1 pt-1">
                                            <button
                                                ...
                                                className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-red-900/20 text-sm text-red-400 hover:text-red-300 transition-colors text-left"
                                            >
