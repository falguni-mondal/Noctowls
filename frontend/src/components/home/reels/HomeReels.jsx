...
const ReelCard = memo(({ reel, isCurrent, onPlay, onPause }) => {
  ...
  return (
    <div className="group relative aspect-9/16 bg-zinc-950 rounded-2xl overflow-hidden border border-zinc-800 shadow-lg hover:shadow-red-900/10 transition-all duration-500 hover:border-red-700/50">
      {isPlaying ? (
        <video
          src={reel.videoUrl}
      ...
      {isPlaying && isBuffering && (
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/40 backdrop-blur-xs">
          <img
            className="w-8 aspect-square opacity-80"
            src={loadingGif}
            alt="loading..."
          />
        </div>
      ...
      <div
        className={`absolute bottom-0 left-0 w-full h-1/4 bg-linear-to-t from-black/80 to-transparent pointer-events-none transition-opacity duration-300 ${isPlaying ? "opacity-40" : "opacity-80"}`}
      ></div>

      <div
        className={`absolute top-4 right-4 bg-black/60 backdrop-blur-md px-2 py-1 rounded-full text-white pointer-events-none transition-all duration-300 ${isPlaying ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2"}`}
      >
