...
const Phase00 = () => {
  ...
  // If the intro has already played, start the hero scrambling immediately
  ...
  useGSAP(
    () => {
      // --- ONLY RUN INTRO ANIMATION IF NOT PLAYED THIS SESSION ---
      if (!hasIntroPlayed) {
        ...
        tl.to(".dial-wheel", {
          ...
          .from(
            ".hero-element",
            {
              ...
              ease: "power3.out",
              onStart: () => {
                // Trigger hero animation & save session state so it skips next time
                setIntroFinished(true);
                sessionStorage.setItem("phase00IntroPlayed", "true");
              }, 
            },
            "-=0.2"
          )
      ...
      // This runs regardless of session storage so the scroll physics always work
    ...
    { scope: containerRef, dependencies: [hasIntroPlayed] }
  );
  ...
  return (
    ...
    >
      ...
      {/* 2. Intro Animation Component (Conditionally Rendered) */}
      {!hasIntroPlayed && <PhaseIntro />}
      
      {/* 3. The Moon & Hero Text Component */}
